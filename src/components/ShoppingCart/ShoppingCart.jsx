...
const ShoppingCart = () => {
  ...
  // Ref to track if rentNowProduct has been processed
  const hasProcessedRentNow = useRef(false);

  // Handle rentNowProduct from navigation state only once
  useEffect(() => {
    const rentNowProduct = location.state?.rentNowProduct;
    if (
      rentNowProduct &&
      !hasProcessedRentNow.current &&
      !cartItems.some((item) => item.productID === rentNowProduct.productID)
    ) {
      dispatch(addToCart({ ...rentNowProduct, quantity: 1 }));
      hasProcessedRentNow.current = true; // Mark as processed
      // Clear location state to prevent re-processing on re-render
      window.history.replaceState({}, document.title, window.location.pathname);
    }
  }, [location.state, cartItems, dispatch]);
  ...
  const handleQuantityChange = (productId, quantity) => {
    if (quantity >= 1 && quantity <= 20) {
      ...
      dispatch(removeFromCart(productId)); // Ensure removal when quantity < 1
    }
  };
  ...
  const handlePlaceOrder = () => {
    const orderData = {
      ...
      totalPrice:
        totalPrice === 0 ? 0 : totalPrice * selectedDates.size + 20000,
    ...
    try {
      const existingOrders = JSON.parse(
        localStorage.getItem('orderProductData') || '[]'
      );
                ...
                <div className='checkoutDetailsSection'>
                  ...
                  <div className='checkoutDetailsForm'>
                    <form>
                      ...
                      <div className='checkoutDetailsFormCheck'></div>
                      <textarea
                        cols={30}
                        rows={8}
                        ...
                        <tbody>
                          ...
                          <tr>
                            ...
                            <td>
                              {totalPrice === 0
                                ? 0
                                : totalPrice * selectedDates.size + 20000}{' '}
                              VND
                            </td>
                          </tr>
                        </tbody>
